...
> 来自 benchmark.md 的对标账号 樊凡说（N=10 样本，import on 2026-07-07）。
> **仅定性方向，不直接采纳为数值权重**——样本拟合容易过拟合，且本批样本无高/低对照组（只能看"高表现共性"）。
> 等你自己 N≥5 校准样本后正式 bump 时**再决定**是否调权重。

- 看起来重要的维度: **HP**（开篇高概念反差钩子、不报片名）/ **NA**（固定结构骨架、时间线单向、多次反转）/ **SR**（结尾强制升华到社会议题）
- 需要重新定义理解的维度: **ER** —— 该形态的共鸣靠"极冷静描述极离谱的事"制造（冷共鸣），打分时不按煽情浓度理解
- 暂无法判断的维度: **QL / AB / SAT**（无对照组，方向未知）
- Claude 给的初始建议: 对标是**高概念钩子 + 结构纪律 + 议题落点**驱动型；打分时 HP/NA/SR 的定义可参照对标风格理解，但权重保持等权直到第一次 bump
